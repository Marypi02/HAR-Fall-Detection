...
def load_har(data_dir: Path, batch_size=32, val_split=0.2, num_workers=4):
    ...
    all_subjects = full_train_dataset.subjects # carichi tutti i soggetti del train
    unique_subjects = np.unique(all_subjects) 

    np.random.seed(42)
    np.random.shuffle(unique_subjects) # mischi l'ordine dei soggetti 

    num_val_subjects = int(len(unique_subjects) * val_split) # definisci quanti soggetti andranno nella Val
